...
| **Last session ended because** | task complete, stopping point reached cleanly — see Session Log. Ran in claude.ai chat with a user-supplied GitHub token again this session; pushed to `origin` directly. |
...
9. Invoice PDF generation (PRD §6.7) — client-side with **pdf-lib** (locked in below, don't reach for anything server-side), customizable logo/address/footer/signature/terms/brand color, plus the Android **Share Sheet** via the Web Share API (`navigator.share` with a `File`, not a custom share UI). Since Phase 0 never assigns a real `invoice_number` (see the Customer/Vehicle Profile screen's "Invoice pending sync" label, task 7), the PDF must use the same device-local reference (`device_local_id`) rather than a fabricated number — re-read §6.7's exact wording on this before writing the PDF header, it's specific about why. Also worth wiring in per §6.7's zero-cost suggestion: a `wa.me/<number>?text=...` pre-filled WhatsApp deep link as a one-tap option alongside the generic Share Sheet, if `billSelectedCustomer.phone` is on file.
