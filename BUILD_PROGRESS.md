...
| **Last session ended because** | task complete, stopping point reached cleanly — see Session Log. Note: this session ran in claude.ai chat, not Claude Code, and has no push credentials — code + this file are committed locally in the session's sandbox clone but may not be pushed to `origin` yet. Check `git log origin/main` vs local before assuming this is on GitHub. |
...
7. Build the Customer/Vehicle profile screen (PRD §6.4) — search by plate/phone/name (ranked: exact plate match > exact phone match > name fuzzy match, per §9's Notable States column), full history in one scroll (vehicles, jobs, invoices, payments). This is reachable today only via the "More" tab (still a placeholder) or a new entry point on top of Jobs/New Bill's existing customer search — pick one and note the choice here if it's not obvious once you're in the code. Once this exists, two things flagged in Blockers below become worth revisiting (not required for this task): (a) New Bill's "bill from this job" path now has a natural home next to a customer's job history, and (b) job status_history / mechanic reassignment history (currently silently overwritten, see Blockers) would have somewhere to actually be displayed.
